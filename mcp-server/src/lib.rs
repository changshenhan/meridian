//! S-13 MCP 服务器正式版：`mist.authorize` / `mist.pay` / `mist.balance` /
//! `mist.attest` / `mist.verify_receipt` / `mist.revocation_witness`（S-52）。
//!
//! 形态：**stdio MCP server + 内嵌真实聚合器内核**（`mist-aggregator`：WAL 持久化、
...
//! 缝，真验证后端（`BbVerifier`，bin `MIST_VERIFY_BACKEND=bb` + S-48 撤销根绑定闸）
...
/// Mist MCP 服务器正式版（S-13）。
#[derive(Clone)]
pub struct MistServer {
...
impl std::fmt::Debug for MistServer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MistServer")
