...
//!
//! 重试判据在类型上分叉：只有 [`SdkError::Transport`] 是重试候选；[`SdkError::Meridian`]
...
pub enum SdkError {
    ...
    /// Meridian 错误码（聚合器拒绝 / 本地构造校验）。`Error::as_code()` 透传，永不重试。
    Meridian(Error),
    /// 本地错误（参数 / 状态，非协议码）。
    Local(String),
}
...
impl SdkError {
    ...
    pub fn code(&self) -> String {
        match self {
            SdkError::Meridian(e) => e.as_code().to_string(),
...
impl fmt::Display for SdkError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SdkError::Meridian(e) => write!(f, "{}", e.as_code()),
