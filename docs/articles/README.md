...
| 05 | [A settlement epoch on Base mainnet, end to end](05-mainnet-settlement-en.md) | The first full optimistic-settlement life cycle on a public mainnet with real money: commit (bond escrowed) → settle → 6-hour window (zero challenges) → claim → `releaseBond`. Mainnet runs +10–11% over the anvil-fitted coefficients; at N=5 the batch is ~38% more expensive than direct — the run bought the life cycle, not the margin. |
