...
| 砖 | 内容 | 规模 |
|---|---|---|
| **L3-0** | **摄取 / apply 分离的可测性收口**：把现有摄取管线抽成纯 apply 函数（输入日志条目序列，输出状态 + 裁决），property test = N 副本乱序投递同一条目集合 → 状态根与裁决史逐字节一致。**不改 WAL 格式、不改链上面**——共享账本的地基，也是对现有单写者栈独立有价值的重构（当前管线与网络 I/O 耦合） | 中 |
...
| **L3-3** | 观察面 / monitor / ops 迁移：绑定闸与 kind4 退役路径、S-39 副本组语义升级、S-65 声誉信源改 QC 事件 | 中 |
