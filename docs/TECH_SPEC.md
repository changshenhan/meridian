...
- **2026-09-02 主网真跑全生命周期收官（S-79，claim+releaseBond 对账完成）**：epoch 0
  ...
  （不归因只报数）——02 文章公式保留 anvil 拟合读作下界并加主网锚点注记（老板拍板，
  commit `7391054`）；claim/release 为主网全 tx 口径（含 21k base + intrinsic），与
  harness execution-only delta 不做直接百分比换算、两口径并列（05 文章承担全表）。
  **运营坑（S-68 同款复发面）**：releaseBond tx 成功落地后，`bondedAmount` 断言读撞
  公共 RPC 负载均衡滞后节点读到旧值 5e14 → 二进制假 panic（exit 101，对账单未打印）。
  链上三面验证全绿（事件 / receipt status=1 / 合约 balance=0），资金零风险、前置幂等
