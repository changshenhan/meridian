...
7. **算术**：净额求和溢出（uint256）、`msg.value ≥ Σnet` 边界、费用为 0/1 wei 的
   极端净额。
8. **挑战押金（S-38）**：押金入场前仅四类 revert（未结算 / 已挑战 / 窗口外 / 金额不等）；
...
- ~~challenge 无押金~~——**S-38 已收口（2026-08-30）**：`challenge` 变 `payable`，押金
  ...
  改为 `ChallengeRejected` 事件 + 押金全额销毁（`address(0)`），epoch 状态不变、仍可再挑战；
  成功路径挑战者拿回押金 + 运营者债券。押金从不停留为合约状态（本笔交易内结清）。
  设计全文见 TECH_SPEC §6.5。残余自报：押金金额为固定常量，未随 gas 价格/债券规模动态化。

## 5. 测试与证据基线

- forge 65/65（含 S-28 USDC 10 例：黑名单/双资产退款/资产隔离；S-38 挑战押金 2 例 +
  既有挑战负向用例改为"驳回即没收"断言）。
